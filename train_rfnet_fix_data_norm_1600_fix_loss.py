...
parser.add_argument('--gpuids', type=str, default="1,2", help='IDs of GPUs to use')
...
parser.add_argument('--name', type=str, default='rfnet_Norm_1600_fix_loss', help='A name to attach to the training session')
...
def depth_loss_function(y_true, y_pred, theta=0.1):
    maxDepthVal = args.maxdepth
    y_true = y_true * SACLE_Y
...
with strategy.scope():
    ...
    parallel_model.compile(loss=depth_loss_function, optimizer=optimizer, metrics=metrics)
