...
class BasicPolicy(object):
    def __init__(self, image_shape, mirror_ratio = 0, flip_ratio = 0, color_change_ratio = 0, is_full_set_colors = False, add_noise_peak = 0.0, erase_ratio = -1.0, scale_ratio=0.0):
        ...
        # self.s = np.random.uniform(1.0, 1.5)  # random scaling
        # self.Resize = Resize(self.s)
        # self.CenterCrop = CenterCrop(image_shape)
    ...
    def __call__(self, img, depth):
        ...
        # 5) Scale
        # if random.uniform(0, 1) < self.scale_ratio:
        #     depth = depth / self.s
