...
class DataGenerator(Sequence):
    ...
    def generate_batch_X_Y(self, batch_list):
        ...
        for indx, (image_path, label_path) in enumerate(batch_list):
            ...
            label = self.DepthNorm(label, self.max_depth)
...
parser.add_argument('--name', type=str, default='rfnet_div', help='A name to attach to the training session')
...
def loss_rmse_DepthNorm(truth, pred, mindepth=args.mindepth, maxdepth=args.maxdepth):
    ...
    mask = tf.where(tf.equal(truth, thresh), zero_shape, one_shape)
...
callbacks = get_nyu_callbacks(parallel_model, train_generator, val_generator, runPath,
                              totaL_epochs=args.epochs, warmup_epoch=5, batch_size=args.bs, lr=args.lr,val_loss="val_loss_sirmse_DepthNorm")
