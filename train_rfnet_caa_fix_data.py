...
from model_rfnet_caa import RFDN
import csv
import cv2
import numpy as np
...
class DataGenerator(Sequence):
    def __init__(self, data_path, min_depth, max_depth, batch_size, image_shape, depth_shape, n_channels, is_augment=False, shuffle=False, mode="train",
        ...
        valid_csv_path = os.path.join(self.data_path, "val.csv")
        if mode == "train":
...
parser.add_argument('--data_path', default='/home/share/competition/depth_estimation', type=str, help='Training dataset.')
parser.add_argument('--lr', type=float, default=0.002, help='Learning rate')
parser.add_argument('--bs', type=int, default=32, help='Batch size')
parser.add_argument('--epochs', type=int, default=100, help='Number of epochs')
parser.add_argument('--gpuids', type=str, default=" ", help='IDs of GPUs to use')
...
with strategy.scope():
    rfanet_x = RFDN(scale_factor=SCALE_VALE)
    x = Input(shape=(int(480/SCALE_VALE), int(640/SCALE_VALE), 3))
    out = rfanet_x(x)
    parallel_model = Model(inputs=x, outputs=out)
    print("FLOPs:",get_flops(parallel_model))

    parallel_model.compile(loss=loss_rmse_baseline, optimizer=optimizer, metrics=metrics)
...
callbacks = get_nyu_callbacks(parallel_model, train_generator, val_generator, runPath,
                              totaL_epochs=args.epochs, warmup_epoch=5, batch_size=args.bs, lr=args.lr,sirmse="val_loss_sirmse_baseline")
