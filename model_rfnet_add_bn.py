...
class ESANet(Model):
    def __init__(self, out_channel):
        ...
        self.bn_max = BatchNormalization(axis=-1)
    ...
    def __call__(self, inputs):
        ...
        v_range = self.relu(self.bn_max(self.conv_max(v_max)))
        c3 = self.relu(self.bn3(self.conv3(v_range)))
        c3 = self.conv3_(c3)
...
def mean_channels(inputs):
    ...
    assert len(dim)==4
    spatial_sum = K.sum(axis=2, keepdims=True)
    spatial_sum = K.sum(axis=1, keepdims=True)
...
class CCA(Model):
    def __init__(self, channel, reduction=16):
        ...
        self.avg_pool = GlobalAveragePooling2D(1)
...
class RFDN(Model):
    ...
    def __call__(self, inputs):
        ...
        out_B = self.act_C(self.bn_C(self.conv_C(out_B)))
